...
describe('useSizeScale', () => {
	...
	describe('sizeScaleStyle при scale=0', () => {
		it('должен устанавливать --size-scale когда scale=0', () => {
			const { sizeScaleStyle } = useSizeScale({ getScale: () => 0 })

			expect(sizeScaleStyle.value).toEqual({ '--size-scale': '0' })
		})
	})

	describe('sizeScaleStyle при scale=null/undefined', () => {
		it('должен возвращать undefined когда getScale возвращает null', () => {
			const { sizeScaleStyle } = useSizeScale({ getScale: () => null as unknown as number })

			expect(sizeScaleStyle.value).toBeUndefined()
		})

		it('должен возвращать undefined когда getScale возвращает undefined', () => {
			const { sizeScaleStyle } = useSizeScale({ getScale: () => undefined as unknown as number })
